    ...
    Protected Sub rptFields_ItemDataBound(ByVal sender As Object, ByVal e As System.Web.UI.WebControls.RepeaterItemEventArgs) Handles rptFields.ItemDataBound
        If e.Item.ItemType = ListItemType.Item Or e.Item.ItemType = ListItemType.AlternatingItem Then
            ...
            Select Case intPropertyTypeId
                ...
                Case 901
                    Dim cblProgramTypes As New CheckBoxList
                    cblProgramTypes.CssClass = "main"
                    cblProgramTypes.RepeatColumns = 3

                    cblProgramTypes.DataSource = Emagine.GetDataReader("SELECT * FROM qryCustom_ProgramTypes WHERE IsEnabled = 'True' ORDER BY SortOrder")
                    cblProgramTypes.DataValueField = "ProgramTypeID"
                    cblProgramTypes.DataTextField = "ResourceName"
                    cblProgramTypes.DataBind()

                    Dim arySelectedValues As Array = strSelectedValue.Split("||")
                    For i As Integer = 0 To arySelectedValues.GetUpperBound(0)
                        For Each Item As ListItem In cblProgramTypes.Items
                            If Item.Value = arySelectedValues(i).ToString Then
                                Item.Selected = True
                            End If
                        Next
                    Next

                    Formfield = cblProgramTypes
                    Formfield.ID = intPropertyID

                Case 902
                    Dim cblDiseases As New CheckBoxList
                    cblDiseases.CssClass = "main"
                    cblDiseases.RepeatColumns = 3

                    cblDiseases.DataSource = Emagine.GetDataReader("SELECT * FROM qryCustom_Diseases WHERE IsEnabled = 'True' ORDER BY SortOrder")
                    cblDiseases.DataValueField = "DiseaseID"
                    cblDiseases.DataTextField = "ResourceName"
                    cblDiseases.DataBind()

                    Dim arySelectedValues As Array = strSelectedValue.Split("||")
                    For i As Integer = 0 To arySelectedValues.GetUpperBound(0)
                        For Each Item As ListItem In cblDiseases.Items
                            If Item.Value = arySelectedValues(i).ToString Then
                                Item.Selected = True
                            End If
                        Next
                    ...
                    Formfield = cblDiseases
                    Formfield.ID = intPropertyID

                Case 903
                    Dim rblIsArchived As New RadioButtonList
                    rblIsArchived.RepeatDirection = RepeatDirection.Horizontal
                    rblIsArchived.Items.Add(New ListItem("Yes", "Yes"))
                    rblIsArchived.Items.Add(New ListItem("No", "No"))

                    If strSelectedValue.Length > 0 Then
                        For Each Item As ListItem In rblIsArchived.Items
                            If Item.Value = strSelectedValue Then
                                Item.Selected = True
                                Exit For
                            End If
                        Next
                    Else
                        rblIsArchived.Items(1).Selected = True
                    End If

                    Formfield = rblIsArchived
                    Formfield.ID = intPropertyID
            End Select
